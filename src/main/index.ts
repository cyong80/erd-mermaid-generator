...
function buildSchema(
  ...
  const tableMap: Record<
    string,
    { name: string; columns: SchemaTable["columns"] }
  > = {};
  ...
  const fkSet = new Set(
    relations.map(
      (r) => `${r.fromTable ?? r.FROM_TABLE}__${r.fromColumn ?? r.FROM_COLUMN}`,
    ),
  );
...
ipcMain.handle("scan-db", async (_, config: DBConfig) => {
  const result =
    config.dbType === "postgres"
      ? await scanPostgres(config)
      : await scanMySQL(config);
...
/** Mermaid ERD type에서 콤마 치환 (DECIMAL(10,2) 파싱 오류 방지) */
...
function schemaToMermaid(
  ...
  for (const rel of schema.relations) {
    ...
    seen.add(key);
    lines.push(
      `  ${rel.from.table} }o--|| ${rel.to.table} : "${rel.from.column}"`,
    );
  }
  return lines.join("\n");
}
ipcMain.handle(
  ...
  (
    _,
    {
      ...
      dbName,
      includedTables,
    }: { schema: Schema; dbName: string; includedTables: string[] | null },
...
ipcMain.handle("delete-recent-connection", (_, id: string) => {
  const list = (store.get("recentConnections") as Array<{ id: string }>).filter(
    (c) => c.id !== id,
  );
...
ipcMain.handle(
  ...
  async (
    _,
    { mermaidCode, dbName }: { mermaidCode: string; dbName: string },
  ) => {
    ...
    const pathToSave = filePath.toLowerCase().endsWith(".md")
      ? filePath
      : `${filePath}.md`;
...
function parseMarkdownFile(
  content: string,
): { mermaidCode: string; dbName: string } | null {
...
ipcMain.handle("load-markdown", async () => {
  ...
  try {
    ...
    if (!parsed)
      return { loaded: false, error: "Mermaid ERD 코드를 찾을 수 없습니다." };
...
ipcMain.handle("clear-obsidian-vault", () =>
  store.set("obsidianVaultPath", ""),
);
...
ipcMain.handle("pick-obsidian-vault", async () => {
  ...
  return {
    picked: true,
    vaultPath,
    isValid: existsSync(join(vaultPath, ".obsidian")),
  };
});
...
ipcMain.handle(
  ...
  async (
    ...
    const vaultPath = store.get("obsidianVaultPath") as string;
    if (!vaultPath)
      return { saved: false, error: "Vault 경로가 설정되지 않았습니다." };
    const erdFolder = subFolder
      ? join(vaultPath, subFolder, "ERD")
      : join(vaultPath, "ERD");
